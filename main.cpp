...
std::string normalizedSchedulerName(std::string name) {
    std::string out;
...
bool isPartitionedScheduler(const std::string& name) {
    const std::string s = normalizedSchedulerName(name);
    return s == "prm" || s == "partitioned" ||
           s == "partitionedrm" || s == "partitionededf" ||
           s == "partitionedcontext" || s == "partitionedctx" ||
           s == "partitionedadaptive" || s == "partitionedhonest" ||
           s == "partitionedcontexthonest" ||
           s == "partitionedttu" || s == "partitionedpredictive";
}

std::unique_ptr<CorePolicy> makePolicy(const std::string& name, bool triage) {
    const std::string s = normalizedSchedulerName(name);
    if (s == "edf" || s == "partitionededf") return makeEdfPolicy();
    if (s == "context" || s == "ctx" || s == "adaptive" ||
        s == "partitionedcontext" || s == "partitionedctx" || s == "partitionedadaptive")
                           return makeContextAwarePolicy();  // oracle (reads *_real)
    if (s == "honest" || s == "contexthonest" || s == "partitionedhonest" ||
        s == "partitionedcontexthonest")
                           return makeContextAwareHonestPolicy();
    if (s == "ttu" || s == "predictive" || s == "partitionedttu" ||
        s == "partitionedpredictive")
...
void usage() {
    std::printf(
        ...
        "  --scheduler rm|edf|context|honest|ttu       scheduling policy (default rm;\n"
        "                               context = oracle, honest = remote metrics only,\n"
        "                               ttu = predictive time-to-unsafe ranking)\n"
        "                               partitioned aliases: partitionedRM, partitionedEDF,\n"
        "                               partitionedContext, partitionedHonest, partitionedTTU\n"
        "                               prm/partitioned = legacy aliases for partitionedRM\n"
        ...
        "  --schedule-viz FILE.svg      write a standalone scheduler timeline SVG\n"
        "  --schedule-viz-ms MS         timeline duration to record (default 120)\n"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        scheduleViz.maxMs = std::atof(argValue(argc, argv, "--schedule-viz-ms", "120"));
        auto scheduler = std::make_unique<PolicyScheduler>(
            makePolicy(schedName, params.triage),
            isPartitionedScheduler(schedName),
            scheduleViz);
